...
class AcademicCompositionEngine:
    ...
    def analyze(self, image_bytes):
        ...
        # 空間幾何三分區域切片（對齊 PhotoFramer 論文空間分割理論） [cite: 325, 373]
        ...
        # ─── 進入 PhotoFramer 論文三大層級任務決策鏈 ─── [cite: 325, 373]
        
        # 任務 1: Shift Task (平移調整) -> 檢查左右權重是否偏斜 [cite: 325, 373]
        if stat_left > stat_right * self.th_ratio:
            raw_action = "left"
            instructions = "【平移調整】請向左平移手機，使主體對齊右側網格線"
        elif stat_right > stat_left * self.th_ratio:
            raw_action = "right"
            instructions = "【平移調整】請向右平移手機，使主體對齊左側網格線"
        else:
            # 任務 2: Zoom-in Task (變焦放大) -> 檢查主體是否過於渺小 [cite: 325, 373]
            if stat_center < stat_global * 0.86:
                raw_action = "zoom_in"
                instructions = "【變焦放大】主體在畫面中偏小，請手動放大焦距或向前走近"
            # 任務 3: View-change Task (視角切換) -> 檢查透視壓迫感 [cite: 325, 373]
            elif stat_center > stat_global * 1.18:
                raw_action = "view_change"
                instructions = "【視角切換】背景產生幾何壓迫感，請退後一步或改變相機仰角"
            else:
                raw_action = "perfect"
                instructions = "畫面結構穩定平衡，符合構圖原則，請直接按下快門"
        ...
        # 經典原生 PhotoFramer 安全快門裁切機制 [cite: 325, 373]
